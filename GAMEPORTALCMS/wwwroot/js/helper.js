...
var helper = function () {
    ...
    var get = function (url, data, callback, async, showLoader) {
    ...
    var _globalEventHandler = function () {
        ...
        $(document).off('keypress', '.percentage-only-sw').on('keypress', '.percentage-only-sw', function (evt) {
            var val1;
            ...
            if (evt.keyCode != 44) {
                var currVal = String.fromCharCode(evt.keyCode);
                val1 = parseFloat(String(parts[0]) + String(currVal));
                if (typeof evt.target.selectionStart == "number" && (evt.target.selectionEnd - evt.target.selectionStart) > 0) {
                    val1 = parseFloat(currVal);
                }
                if (parts.length == 2)
                    val1 = parseFloat(String(parts[0]) + "," + String(currVal));
            }
        ...
        $(document).off('keypress', '.money-only-sw').on('keypress', '.money-only-sw', function (evt) {
            ...
            if (evt.keyCode != 44) {
                var parts = evt.target.value.split(',');
                var currVal = String.fromCharCode(evt.keyCode);
                var val1 = parseFloat(String(parts[0]) + String(currVal));
                if (typeof evt.target.selectionStart == "number" && (evt.target.selectionEnd - evt.target.selectionStart) > 0) {
                    val1 = parseFloat(currVal); return true;
                }
                if (parts.length == 2)
                    val1 = parseFloat(String(parts[0]) + "," + String(currVal)); return true;
            }

            var isOk = true;
            if ($(this).attr('data-max-value')) {
                var value = parseFloat($(this).val()) + parseFloat(String.fromCharCode(charCode));
                var maxvalue = parseFloat($(this).data('max-value'));
                var val2 = parseFloat(value);
                if (val2 > maxvalue) {
                    isOk = false;
